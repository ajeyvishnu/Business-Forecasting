...
```{r}
tsdisplay(diff(cafe,4))
```

* In the Acf plot we can see that the p value is 2 and the q values is 2.
* So, the model takes values of 0,1,2 for p and q.
* In the Pacf plot, we can see that the P value is 1 and the Q value is 2.
* So, the model takes values of 0,1 for P and 0,1,2 values for Q.
